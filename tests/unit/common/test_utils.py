...
try:
    from backend.common.utils import (
        generate_id, format_timestamp, parse_timestamp, validate_required_fields,
        validate_field_type, validate_field_range, validate_enum_field,
        calculate_percentage_change, calculate_moving_average, normalize_data,
        denormalize_data, resample_data, merge_dictionaries, filter_dict_by_keys,
        deep_get, deep_set, flatten_dict, unflatten_dict, dict_to_object, object_to_dict
    )
    ...
    def validate_field_range(value, field_name, min_value=None, max_value=None):
        if min_value is not None and value < min_value:
            raise ValidationError(f"Value for {field_name} is below minimum: {value} < {min_value}")
        if max_value is not None and value > max_value:
            raise ValidationError(f"Value for {field_name} is above maximum: {value} > {max_value}")
    
    def validate_enum_field(value, field_name, valid_values):
        if value not in valid_values:
            raise ValidationError(f"Invalid value for {field_name}: {value}. Valid values: {', '.join(valid_values)}")
