...
class MockResponse:
    ...
    def raise_for_status(self):
        ...
        if self.status_code >= 400:
            raise requests.exceptions.HTTPError(f"Mock HTTP Error: {self.status_code} {self.reason}")
...
class APIMockFactory:
    ...
    def create_market_data_response(
        ...
        # Generate timestamps
        timestamps = pd.date_range(start=start_date, periods=periods, freq=timeframe.replace('1d', 'D').replace('1h', 'H'))
...
class MockAPIClient:
    ...
    def get(self, endpoint: str, params: Optional[Dict[str, Any]] = None) -> MockResponse:
        """
        Mock GET request.
        ...
        # Default response
        return MockResponse(status_code=404, json_data={'error': 'Not Found', 'message': 'Endpoint not found'}, reason="Not Found")
    
    def post(self, endpoint: str, data: Optional[Dict[str, Any]] = None) -> MockResponse:
        """
        Mock POST request.
        ...
        # Portfolio update endpoint
            ...
            if len(parts) >= 2:
                ...
                return APIMockFactory.create_portfolio_response(
                    portfolio_id=portfolio_id,
                    user_id="user_1234567890",
                    name=data.get('name', "Test Portfolio") if data else "Test Portfolio",
