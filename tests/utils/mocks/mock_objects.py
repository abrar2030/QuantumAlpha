...
class MockModelManager:
    ...
    def predict(
        ...
        for i in range(horizon):
            ...
            predictions.append({
                'timestamp': (datetime.utcnow() + timedelta(days=i+1)).strftime('%Y-%m-%dT%H:%M:%SZ'),
        ...
        return {
            ...
            'prediction': {
                'average': round(sum(p['value'] for p in predictions) / len(predictions), 2),
                ...
                'change_percent': round((predictions[-1]['value'] - latest_price) / latest_price * 100, 2),
                'direction': 'up' if predictions[-1]['value'] > latest_price else 'down'
            },
...
class MockBrokerAPI:
    ...
    def place_order(
        ...
        if side == 'buy':
            ...
            position['average_price'] = (position['average_price'] * (position['quantity'] - quantity) + (price or 100.0) * quantity) / position['quantity']
        else:
            position['quantity'] -= quantity
