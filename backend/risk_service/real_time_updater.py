        ...
        def process_data():
            while self.running:
                try:
                    ...
                    self._trigger_callbacks('error_occurred', {'error': str(e), 'context': 'data_processing'})
            ...
            while self.running:
                try:
                    ...
                    if market_data:
                        ...
                        logger.info(f"Models updated: {result.get('models_updated', [])}")
                
                except Exception as e:
                    logger.error(f"Error in model updater: {e}")
                    self.metrics['processing_errors'] += 1
                    self._trigger_callbacks('error_occurred', {'error': str(e), 'context': 'model_updating'})
        ...
        def detect_anomalies():
            while self.running:
                try:
                    ...
                    # Detect anomalies
                    if market_data:
                        result = self.online_learning_engine.detect_market_anomalies(market_data)
                        
                        if result.get('anomalies_detected', False):
                            self.metrics['anomalies_detected'] += len(result.get('anomalous_symbols', []))
                            
                            # Trigger callbacks
                            self._trigger_callbacks('anomaly_detected', result)
                            
                            logger.warning(f"Anomalies detected: {result.get('anomalous_symbols', [])}")
                
                except Exception as e:
                    logger.error(f"Error in anomaly detector: {e}")
                    self.metrics['processing_errors'] += 1
                    self._trigger_callbacks('error_occurred', {'error': str(e), 'context': 'anomaly_detection'})
                ...
                try:
                    ...
                    for symbol in self.tracked_symbols:
                        try:
                            ...
                            if data['data']:
                                latest_data = data['data'][-1]  # Get most recent data point
                                data_point = {
                                    'symbol': symbol,
                                    'data': latest_data,
                    ...
                    self.metrics['processing_errors'] += 1
                    self._trigger_callbacks('error_occurred', {'error': str(e), 'context': 'data_fetching'})
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'buffer_sizes': {
                ...
                'trade_buffer': sum(len(trades) for trades in self.trade_buffer.values())
            },
            'retrieved_at': datetime.utcnow().isoformat()
        }
