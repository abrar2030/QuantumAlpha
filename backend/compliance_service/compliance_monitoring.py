...
class ComplianceMonitor:
    ...
    def check_compliance(
        ...
        try:
            ...
            for rule_id, rule in self.rules.items():
                ...
                try:
                    result = self._check_rule(rule, portfolio_data, position_data, market_data)
    ...
    def add_violation_callback(self, callback: Callable[[ComplianceViolation], None]) -> None:
    ...
    def _check_rule(
        ...
        try:
            ...
            violation = self._evaluate_threshold(current_value, rule.threshold, rule.operator)
    ...
    def _get_measurement_value(
        ...
        try:
            if measurement_field == 'total_portfolio_value':
                ...
                if not position_data:
                    return 0
                return self._calculate_max_sector_concentration(position_data, portfolio_data)
                ...
                if not position_data:
                    return 0
                return sum(
                    pos.get('notional_value', 0) for pos in position_data
    ...
    def _is_within_grace_period(self, rule: ComplianceRule) -> bool:
        ...
        for violation in self.active_violations.values():
            if violation.rule_id == rule.rule_id:
                ...
                return time_since_detection.total_seconds() < (rule.grace_period_minutes * 60)
    ...
    def _handle_violation(self, violation: ComplianceViolation) -> None:
        ...
        # Log violation
        logger.warning(f"Compliance violation detected: {violation.rule_name} - {violation.description}")
    ...
    def _check_position_rules(
        ...
        logger.debug(f"Checking position rules for {position_data.get('symbol', 'unknown')}")
...
class ComplianceReporter:
    ...
    def generate_compliance_report(
        ...
        period_violations = [
            v for v in self.compliance_monitor.violation_history
        ...
        return {
            ...
            'summary': {
                ...
                'resolution_rate': len(resolved_violations) / total_violations if total_violations > 0 else 0,
