...
class PredictionService:
    ...
    def generate_signals(
        ...
        try:
            ...
            if strategy == 'prediction':
                return self._generate_prediction_signals(symbols, model_id, timeframe, period)
            elif strategy == 'technical':
                return self._generate_technical_signals(symbols, timeframe, period)
            elif strategy == 'ensemble':
                return self._generate_ensemble_signals(symbols, model_id, timeframe, period)
            else:
            ...
            try:
                ...
                if prediction_signal and technical_signal:
                    ...
                    if prediction_signal['type'] == technical_signal['type'] == 'buy':
                        signal_type = 'buy'
                        strength = (prediction_signal['strength'] + technical_signal['strength']) / 2
                    elif prediction_signal['type'] == technical_signal['type'] == 'sell':
                        signal_type = 'sell'
                        strength = (prediction_signal['strength'] + technical_signal['strength']) / 2
                    elif prediction_signal['type'] == 'buy' and technical_signal['type'] == 'sell':
                        signal_type = 'hold'
                        strength = 0.0
                    elif prediction_signal['type'] == 'sell' and technical_signal['type'] == 'buy':
                        signal_type = 'hold'
                        strength = 0.0
                    else:
                        signal_type = 'hold'
    ...
    def get_prediction_history(
        ...
        """
        try:
            logger.info(f"Getting prediction history for {symbol} using model {model_id}")
        ...
        try:
            ...
            for row in result:
                metrics.append({
                    ...
                    'avg_abs_error': float(row['avg_abs_error']) if row['avg_abs_error'] else None,
                    'std_error': float(row['std_error']) if row['std_error'] else None,
                    'min_error': float(row['min_error']) if row['min_error'] else None,
