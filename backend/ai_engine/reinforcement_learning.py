...
class TradingEnvironment(gym.Env):
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            # Get current step
            step = self.history[-1] if self.history else {
                'step': self.current_step,
                'action': 0,
...
class ReinforcementLearning:
    ...
    def __init__(self, config_manager, db_manager):
        ...
        self.model_dir = config_manager.get('ai_engine.rl_model_dir', '/home/ubuntu/quantumalpha_backend/rl_models')
    ...
    def train_model(
        ...
        try:
            ...
            if model_info['algorithm'] == 'ppo':
                result = self._train_ppo_model(model_id, model_info, processed_data, data)
            elif model_info['algorithm'] == 'a2c':
                result = self._train_a2c_model(model_id, model_info, processed_data, data)
            elif model_info['algorithm'] == 'dqn':
                result = self._train_dqn_model(model_id, model_info, processed_data, data)
            elif model_info['algorithm'] == 'sac':
                result = self._train_sac_model(model_id, model_info, processed_data, data)
            else:
                raise ValidationError(f"Unsupported algorithm: {model_info['algorithm']}")
    ...
    def _train_ppo_model(
        ...
        try:
            ...
            # Evaluate model
            mean_reward, std_reward = self._evaluate_rl_model(model, env, n_eval_episodes=10)
    ...
    def predict(
        ...
        try:
            ...
            result = {
                ...
                'return': (history[-1]['portfolio_value'] - params.get('initial_balance', 10000)) / params.get('initial_balance', 10000),
                'actions': actions,
                'history': history
            }
