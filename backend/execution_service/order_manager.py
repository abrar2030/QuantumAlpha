...
class OrderManager:
    ...
    def __init__(self, config_manager, db_manager, broker_integration, execution_strategy):
    ...
    def get_order(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get executions
            executions = session.query(Execution).filter(Execution.order_id == order_id).all()
    ...
    def create_order(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if data['type'] in ['stop', 'stop_limit'] and 'stop_price' not in data:
                raise ValidationError("Stop price is required for stop and stop_limit orders")
    ...
    def _submit_order(self, order: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not broker_id:
                # Use default broker
                broker_id = self.config_manager.get('execution.default_broker_id', 'alpaca')
            ...
            if not execution_strategy_id:
                ...
                execution_strategy_id = self.config_manager.get('execution.default_strategy_id', 'market')
    ...
    def add_execution(
        ...
        try:
            ...
            # Update order status
            filled_quantity = sum(e.quantity for e in session.query(Execution).filter(Execution.order_id == order_id).all()) + quantity
