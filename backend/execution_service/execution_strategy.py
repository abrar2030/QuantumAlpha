...
class ExecutionStrategy:
    ...
    def execute_strategy(
        ...
        try:
            ...
            if strategy_id == 'market':
                return self._execute_market_strategy(order, broker_integration, broker_id)
            elif strategy_id == 'limit':
                return self._execute_limit_strategy(order, broker_integration, broker_id)
                ...
                return self._execute_iceberg_strategy(order, broker_integration, broker_id)
    ...
    def _execute_iceberg_strategy(
        ...
        try:
            ...
            # Validate parameters
            if 'parameters' not in order:
                raise ValidationError("Parameters are required for iceberg order strategy")
            
            if 'display_size' not in order['parameters']:
                raise ValidationError("Display size is required for iceberg order strategy")
