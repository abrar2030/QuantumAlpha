...
class StrategyService {
  async getActiveStrategies() {
    try {
          ...
          resolve({
            data: [
              ...
              },
              {
                id: 'strat3',
                name: 'ML Predictor',
                description: 'Uses machine learning to predict short-term price movements',
                performance: -0.8,
                risk: 'High',
                status: 'Active',
                allocation: 20,
              ...
              },
              {
                id: 'strat4',
                name: 'Value Investor',
                description: 'Identifies undervalued assets based on fundamentals',
                performance: 0.9,
                risk: 'Low',
                status: 'Active',
                allocation: 15,
              ...
              },
              {
                id: 'strat5',
                name: 'Trend Follower',
                description: 'Follows established market trends with adaptive entry/exit',
                performance: 1.2,
                risk: 'Medium',
                status: 'Active',
                allocation: 10,
      ...
      throw new Error(error.response?.data?.message || 'Failed to fetch strategy data');
    }
  }
        ...
        setTimeout(() => {
          const strategies = {
            strat1: {
              id: 'strat1',
              name: 'Momentum Alpha',
              description: 'Captures price momentum across multiple timeframes with adaptive position sizing based on volatility. Uses a combination of technical indicators to identify strong trends.',
              performance: 2.3,
              risk: 'Medium',
              status: 'Active',
              allocation: 30,
            ...
            strat2: {
              id: 'strat2',
              name: 'Sentiment Trader',
              description: 'Analyzes market sentiment from news and social media using natural language processing to identify trading opportunities before they become apparent in price action.',
              performance: 1.5,
              risk: 'Medium-High',
              status: 'Active',
              allocation: 25,
            ...
            strat3: {
              id: 'strat3',
              name: 'ML Predictor',
              description: 'Uses machine learning to predict short-term price movements based on historical patterns, market microstructure, and alternative data sources.',
              performance: -0.8,
              risk: 'High',
              status: 'Active',
              allocation: 20,
            ...
            strat4: {
              id: 'strat4',
              name: 'Value Investor',
              description: 'Identifies undervalued assets based on fundamentals using a combination of financial metrics and qualitative analysis.',
              performance: 0.9,
              risk: 'Low',
              status: 'Active',
              allocation: 15,
            ...
            strat5: {
              id: 'strat5',
              name: 'Trend Follower',
              description: 'Follows established market trends with adaptive entry/exit points and dynamic position sizing based on trend strength.',
              performance: 1.2,
              risk: 'Medium',
              status: 'Active',
              allocation: 10,
  ...
  async updateStrategyAllocation(strategyId, allocation) {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to update strategy allocation');
    }
  }
  ...
  async getAvailableStrategies() {
    try {
      ...
      throw new Error(error.response?.data?.message || 'Failed to fetch available strategies');
    }
  }
}
